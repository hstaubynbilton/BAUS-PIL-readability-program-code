...
def replace_char(text):
    ...
    def list_replacer(match_obj):
        ab = (match_obj.group())
        if match_obj.group() is not None:
            print("§BETWEEN±: " + str(ab))
            return str(match_obj.group()) + '. '
    identifier = r'Between .{0,35}(\n)?.{0,35}patients' #removed .{0,70} from the end
    ...
    text = re.sub(r'(\.)(\s)*(\.)+', '. ', text)
    text = re.sub(r'(\.)(\s)*(\.)+', ' ', text)
    # text = re.sub(r'BAUS\(BAUS\) *(Limited)? *\(?',' BAUS ',text)
...
#         print(r'%%ALL CAPS MATCHED%%')
#         print(ans)
#         print(r'%%end ALL caps%%')
#         return ''
...
def remove_header(text):
    ...
    # text = re.sub(header14_1, '', text)
    # text = re.sub(header14_2, '', text)
    # text = re.sub(header16, '', text)
    return text
...
def remove_footer(text):
    ...
    #
    # if footer_1 in text:
    #     text = text.replace(footer_1, '\n-----')
    # if footer_2 in text:
    #     text = text.replace(footer_2, '\n-----')
    # if footer_3 in text:
    #     text = text.replace(footer_3, '\n-----')
    # if footer_4 in text:
    #     text = text.replace(footer_4, '\n-----')
    # if footer_5 in text:
    #     text = text.replace(footer_5, '\n-----')
...
# if ans == "INFORMATION FOR PATIENTS " or "FREQUENTLY ASKED QUESTIONS ":
#     return ans
# elif ans == "PROCEDURE SPECIFIC INFORMATION FOR PATIENTS ":
#     return ans
# elif ans == "N H S E" or "U T I" or "M D T" or "N H S I":
#     return ans
# else:
#  old_caps_identifier = r'([A-Z]{1,15} )([A-Z]{1,15} )([A-Z| ]{1,67})'
#    all_caps_identifier = r'([A-Z][^a-z]{1,15}[ -\(\)]{0,3}){2,15}(\n)'
#     v2= r'([^a-z]{1,20}\W)([^a-z]{1,15} )([^a-z]{1,17})'
